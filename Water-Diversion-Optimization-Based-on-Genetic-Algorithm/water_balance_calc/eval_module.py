...
@jit
def eval_func(hydro_year, year, month, date, ind_lif_demand_reserv, ind_lif_supply, irr_demand_reserv, irr_supply,irr_demand_total, mon_period=60, date_period=21915):
    
    
    # Water shortage in living and industry
    ind_lif_lack=ind_lif_supply - ind_lif_demand_reserv
    
    # Destruction depth of water shortage in living and industry
    ind_lif_lack_ext=[]
    ...
    # Monthly irrigation water shortage
    irr_lack_mon=[]
    ...
    # Total monthly irrigation water demand
    ...
    # Destruction depth of monthly water shortage in living and industry
    irr_lack_mon_ext=[]
    ...
    # Yearly destructive irrigation water shortage
    irr_lack_yr=[]
    ...
    # Monthly irrigation guarantee rate
    irr_mon_rate=(mon_period-len(np.where(irr_lack_yr < 0)[0]))/(mon_period+1)
    
    # Daily living and industry guarantee rate
...
@jit
def eval_func_opt(hydro_year, year, month, date, ind_lif_demand_reserv, ind_lif_supply, irr_demand_reserv, irr_supply,irr_demand_total, mon_period=60, date_period=21915):
    ...
    # Destruction depth of monthly water shortage in irrigation
    irr_lack_mon_ext=[]
    for i, j, k in zip(irr_lack_mon, irr_demand_mon_total, date):
        if k == 1:
            if j !=0:
