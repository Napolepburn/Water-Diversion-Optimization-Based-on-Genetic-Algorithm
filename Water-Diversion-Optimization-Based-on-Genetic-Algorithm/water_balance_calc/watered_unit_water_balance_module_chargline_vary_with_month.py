...
def reservior_water_balance(input, engineering_param, charging_line, node, total_inflow, design_diversion, 
                            available_diversion, charging_line_disc_rate=[1.0]*12, lim_irr_storage_disc_rate=1.0, lim_ind_lif_storage_disc_rate=1.0, 
    ...
    charg_line = charging_line[node,int(input[1]-1)]*charging_line_disc_rate[int(input[1]-1)];

    if (input[1] <= flood_end) and (input[1] >= flood_start):            
        is_flood_season = 1;
    else:
        is_flood_season = 0;
        
        
    if (input[1] <= eco_end) and (input[1] >= eco_start):            
        is_eco_season = 1;
    else:
        is_eco_season = 0;
    ...
    if storage==0:
        storage = engineering_param[node,3];  
    else:
        pass

    loss=storage*0.02/30.4  

    if eco_demand_vary == 0:                                                                        
    ...
    net_inflow=max(0, input[4]+input[6]+input[7]+input[8]-loss-ecological_demand)  

    if into_reserv_or_not==1:                                                                  
        irr_demand_reserv = input[3];
        theoretical_diversion=0;
        practical_diversion=min(design_diversion*8.64*div_loss_rate, available_diversion*div_loss_rate, max(0, charg_line-storage-net_inflow+irr_demand_reserv));                                         
    else:
        if storage+net_inflow > charg_line:                                                                     
            ...
            irr_demand_reserv = - (practical_diversion - input[3]);                                              
        else:
    ...
    if into_reserv_or_not == 1:
        if storage < limited_ind_lif_storage:                                                                                                      
            ind_lif_supply = min(storage + net_inflow + other_modul_water_source+practical_diversion - dead_storage, ind_lif_rate * ind_lif_demand_reserv);                                                                                               
        else:
            ind_lif_supply = min(storage + net_inflow + other_modul_water_source+practical_diversion - dead_storage, ind_lif_demand_reserv);
    else:
        if storage < limited_ind_lif_storage:                                                                                                 
            ind_lif_supply = min(storage + net_inflow + other_modul_water_source - dead_storage, ind_lif_rate * ind_lif_demand_reserv);                                                                                              
        else:
    ...
    if into_reserv_or_not == 1:
        if storage < limited_irr_storage:
            irr_supply = min(storage + net_inflow + other_modul_water_source + practical_diversion - ind_lif_supply - dead_storage, max(0, irr_rate * input[3]));                                     
        else:
